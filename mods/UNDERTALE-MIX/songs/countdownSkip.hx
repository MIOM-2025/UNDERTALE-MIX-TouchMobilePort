...
function create() {
    ...
    // 默认排除列表（所有模式都跳过倒计时）
    if (songName == 'true-reset' || songName == 'vent' || songName == 'temperate' || songName == 'temperate-cmix') {
        excluded = true;
    }

    // string-theory 仅在玩家模式（非对手模式）下跳过
    if (songName == 'string-theory' && !PlayState.opponentMode) {
        excluded = true;
    }
