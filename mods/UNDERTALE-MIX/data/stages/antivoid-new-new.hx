...
import funkin.savedata.FunkinSave;   // 新增：用于判断歌曲是否解锁
...
function postCreate() {
	...
	if (songExists) {
		...
		var botplayEnabled = Reflect.hasField(FlxG.save.data, prefix + 'botplay') ? Reflect.field(FlxG.save.data, prefix + 'botplay') : false;
...
function MstrumChange() {
    if (!FlxG.save.data.middleScroll) return;

    var opponentMode = PlayState.opponentMode;

    for (strumLine in strumLines.members) {
        var isOpponent = strumLine.opponentSide;
        var hidden = (opponentMode && !isOpponent) || (!opponentMode && isOpponent);
        ...
        for (strum in strumLine) {
            ...
            if (hidden) {
                // 隐藏的箭头 → 两侧（对方箭头位置）
                if (strum.ID < 2) {
                    strum.x = 82 + strum.ID * 112;
                } else {
                    strum.x = FlxG.width - 309 + (strum.ID - 2) * 112;
                }
            } else {
                // 可见的箭头 → 居中
                strum.x = middle[strum.ID];
