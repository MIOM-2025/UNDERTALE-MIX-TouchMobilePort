...
function update(elapsed:Float) {
	...
	if (canPress) {
		...
		// 对话结束，点击淡出对话并弹出菜单
			...
			if (FlxG.mouse.justPressed || controls.ACCEPT) {
				hideDialogueThenShowMenu();   // 改为先淡出对话
			}
		}
		// 保留 BACK 键直接退出（可选）
		if (controls.BACK && !menuShown) {
			endScreen(true);
		}
	}
...
function showMenu() {
	...
	var fontName:String = "undertale-outline";   // 黑边字体
	...
	new FlxTimer().start(0.5, function(_) {
		...
		menuYes.height = 75;
		menuYes.x = centerX - spacing - 125 / 2;
	...
	new FlxTimer().start(1.0, function(_) {
		...
		menuNo.height = 75;
		menuNo.x = centerX + spacing - 125 / 2;
