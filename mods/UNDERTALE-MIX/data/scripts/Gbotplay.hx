...
// 根据 opponentMode 动态获取当前应控制的音轨（与 strumChange 中可见侧一致）
function getTargetStrumLine():StrumLine {
    // 遍历所有 strumLine，找到 opponentSide 与当前 opponentMode 匹配的那个
   
    // 保底：如果没找到，返回默认（可能是玩家侧）
    return PlayState.opponentMode ? strumLines.members[2] : strumLines.members[3];
}
